...
let get_wallet_address (p, s : (get_wallet_address_param * Storage.t)) : SmartWallet.sw_address option = 
  let addr : SmartWallet.sw_address option = Big_map.find_opt (p.nft_address, p.nft_id) s.owner_wallet_map in
  addr 

let balance_of_query (p,s : (balance_of_query_param * Storage.t)) : return = 
  let slf_address : address = Tezos.get_self_address() in
  let cb_opt : NftToken.balance_of_response list contract option = Tezos.get_entrypoint_opt "%balanceOfResponse" slf_address in
  ...
  let fa2 : NftToken.balance_of contract option = Tezos.get_entrypoint_opt "%balance_of" p.nft_address in
  let q_op = match fa2 with
    | None -> (failwith "NO_BALANCE_OF_ENTRYPOINT" : operation)
    | Some ep -> Tezos.transaction bp 0mutez ep in
  let new_recent_balance_requests = Big_map.update (p.withdraw_fa12.withdrawer, (p.withdraw_fa12.nft_id: nat)) (Some p.nft_address) s.recent_balance_requests in
  let new_withdraw_requests = Big_map.update (p.withdraw_fa12.withdrawer, (p.withdraw_fa12.nft_id: nat)) (Some p.withdraw_fa12) s.withdraw_requests in
